...
// hidden bit: 1 for normals, 0 for denormals/zero
  ...
  // absolute value of shiftAmount <=0
  val negShift = Module(new nBitAdderSubtractor(8))
  negShift.a          := zero
  negShift.b          := shiftAmount
  negShift.enable_sub := true.B

  val RightShiftAmount = negShift.sum // shift amount < 0

  // extend significant bcz shift can extend 23 bit to 31 bit signifcand atmost
