...
class FloatingPointAddition extends Module {
  ...
  // ─────────────────────────────────────────────
  // 2. Compare exponents
  //    Find which is larger and compute the difference.
  //    The number with the smaller exponent needs its
  //    significand right-shifted by (expLarge - expSmall).
  // ─────────────────────────────────────────────
  ...
  // expLarge = whichever exponent is larger (used as result exponent base)
  ...
  // ─────────────────────────────────────────────
  // 3. Align mantissas
  //    Right-shift the significand of the smaller-exponent number
  //    by expDiff positions. The larger-exponent significand
  //    stays unchanged. Both are extended to 25 bits so there
  //    is room for a carry bit from addition.
  //
  //    We use a 5-stage right barrel shifter on the smaller sig.
  //    Max useful shift is 24 (beyond that all bits are zero),
  //    so bits [4:0] of expDiff are sufficient.
  //
  //    sigLarge = significand of the number with larger exponent
  //    sigSmall = significand of the number with smaller exponent
  //               after being right-shifted into alignment
  // ─────────────────────────────────────────────
  ...
  // Right barrel shifter on sigSmall25ext (5 stages: 1/2/4/8/16)
  ...
  //    Same sign  → add  (sigLarge + sigSmallAligned)
  //    Diff sign  → subtract (sigLarge - sigSmallAligned)
  //                 sigLarge always >= sigSmallAligned after alignment,
  ...
  //    Diff sign:  result sign = sign of the larger-magnitude input
  //                (the one with the larger exponent, or signA if equal)
  //
  ...
  // ─────────────────────────────────────────────
  // 6. Normalize
  //    Case A — carry out (bit 24 of sumRaw = 1):
  //      This happens only on addition (same sign).
  //      Result = sumRaw >> 1,  exponent = expLarge + 1
  //
  //    Case B — no carry, result already has 1 in bit 23:
  //      No shift needed. exponent = expLarge.
  //
  //    Case C — subtraction produced leading zeros (bit 23 = 0):
  //      Left-shift until bit 23 = 1, decrement exponent by shift count.
  //      We do this with a 5-stage left barrel shifter controlled by
  //      finding the position of the leading 1 in sumRaw[23:0].
  //
  //    The task guarantees the result is a normal number, so we don't
  //    need to handle the result being zero or denormal.
  //
  //    We detect the leading-1 position using a priority encoder:
  //    check bits 23 down to 0, find highest set bit.
  //    The number of left-shifts needed = 23 - position_of_leading_1.
  //
  //    For the carry case we just need 1 right-shift.
  //    For the subtraction case we need up to 23 left-shifts.
  //
  ...
  // Find how many leading zeros are above bit 23 in sumRaw[23:0].
  // We build a 5-bit shift amount using a priority encoder that
  // checks bits 23..0 from high to low.
  //
  // shiftNeeded: the number of left-shifts to bring the leading 1 to bit 23.
  // We encode this as a 5-bit number built from OR/AND chains.
  //
  // Strategy: for each possible shift amount s (0..23), check whether
  // bit (23-s) is the leading 1. We use a Mux chain (priority):
  // if bit23=1 → shift 0
  // else if bit22=1 → shift 1
  // else if bit21=1 → shift 2
  // ...
  // else if bit0=1  → shift 23
  //
  // We represent shiftNeeded as an 8-bit wire (same width as exponent arithmetic).
  //
  // To build this without Scala math/when: we use a chain of Mux(8) modules.
  // Starting from the lowest priority (bit 0 → shift 23), each stage
  // overrides the previous if a higher-priority bit is set.
  ...
  for (i <- 0 until 8) {
    result(i + 23) := normalizedExp(i)
  }
  for (i <- 0 until 23) {
    result(i) := resultMantissa(i)
  }
}
}
